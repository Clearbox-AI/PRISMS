...
class SMToyUnet(nn.Module):
    ...
    def forward(self, x, t):
        ...
        # 2) Downsample
        x_d1, skip1 = self.down1(x, t_emb)  # => [B,64,32,32], skip1 => [B,64,64,64]
        x_d2, skip2 = self.down2(x_d1, t_emb)  # => [B,128,16,16], skip2 => [B,128,32,32]
        x_d3, skip3 = self.down3(x_d2, t_emb)  # => [B,256,8,8],   skip3 => [B,256,16,16]

        # 3) Bottleneck
        x_m = self.mid(x_d3, t_emb)  # => [B,256,8,8]

        # 4) Upsample
        x_u3 = self.up3(x_m, skip3, t_emb)  # => [B,128,16,16]
        x_u2 = self.up2(x_u3, skip2, t_emb)  # => [B,64,32,32]
        x_u1 = self.up1(x_u2, skip1, t_emb)  # => [B,64,64,64]

        # 5) Final 1×1 conv => [B,3,64,64]
        return self.final_conv(x_u1)


if __name__ == "__main__":
    # Quick test
    model = SMToyUnet()
    dummy_img = torch.randn(8, 3, 64, 64)  # batch of 8
    dummy_t = torch.randint(0, 1000, (8,))  # random timesteps in [0, 999]

    out = model(dummy_img, dummy_t)
    print("Output shape:", out.shape)  # should be [8, 3, 64, 64]
