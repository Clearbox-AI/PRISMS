...
def load_training_data(cfg: DictConfig):
    ...
    elif dataset_type == "nacc_adapter":
        dataset = NaccDatasetForAdapter(
            data_dir=cfg.data.data_dir
        )
    else:
...
class NaccDataset(Dataset):
    ...
    def __init__(
        ...
        if self.do_augment:
            self.image_transform = Compose([
                ...
                RandRotate(range_x=15, prob=0.5),  # 3D rotation
    ...
    def __getitem__(self, idx):
        ...
        # 8) Convert to torch tensor if not already one
        if not th.is_tensor(image):
    ...
    def _augment_image(self, image: np.ndarray) -> np.ndarray:
        """
                Apply MONAI augmentations to the image.
                Note: This assumes the image is in the expected format by the transforms (e.g. channel-first).
                """
        # Apply the augmentation pipeline from MONAI
...
class NaccDatasetForAdapter(Dataset):
    ...
    def _get_data(self, patient_dir: str):
        # Define paths to the files for the patient
        condition_tab_path = os.path.join(patient_dir, "condition_tab.json")
        dit_tabular_path = os.path.join(patient_dir, "dit_tabular.json")
        latents_dit_path = os.path.join(patient_dir, "latents_dit.npy")
        latents_original_path = os.path.join(patient_dir, "latents_original.npy")

        # Load JSON files
        with open(condition_tab_path, "r") as f:
            condition_tab = json.load(f)
        with open(dit_tabular_path, "r") as f:
            dit_tabular = json.load(f)

        # Load NumPy arrays
        latents_dit = np.load(latents_dit_path, mmap_mode="r")
        latents_original = np.load(latents_original_path, mmap_mode="r")

        return condition_tab, dit_tabular, latents_dit, latents_original

    def __getitem__(self, idx):
        # Retrieve the patient directory corresponding to idx
        patient_dir = self.patient_dirs[idx]

        # Use _get_data to load the patient's data
        condition_tab, dit_tabular, latents_dit, latents_original = self._get_data(patient_dir)

        return {
            "latents_dit": latents_dit,
            "latents_original": latents_original,
            "condition_tab": condition_tab,
            "dit_tabular": dit_tabular
        }
