...
def main(args):
    """
    1. Load the MRI image from args.image_path and display
    2. Normalize and display
    3. Convert to 2-channel and display (real & imag)
    4. Encode -> visualize latent
    5. Decode -> visualize reconstruction
    """
    # -----------------------------------------------------------------
    ...
    fig.suptitle("Step-by-step Autoencoder Encoding/Decoding")

    # Display original magnitude (step 1)
    axs[0, 0].imshow(np.abs(coil_complex_image[0]), cmap="gray")
    axs[0, 0].set_title("1) Original Magnitude")
    axs[0, 0].axis("off")
    ...
    # Display normalized magnitude
    axs[0, 1].imshow(np.abs(coil_complex_image_norm[0]), cmap="gray")
    axs[0, 1].set_title("2) Normalized Magnitude")
    axs[0, 1].axis("off")
    ...
    # Show real/imag parts separately (just as a quick check):
    # two_channel_image shape => [1, 2, H, W]
    real_part = two_channel_image[0, 0]  # shape [H, W]
    imag_part = two_channel_image[0, 1]  # shape [H, W]

    axs[1, 0].imshow(real_part, cmap="gray")
    axs[1, 0].set_title("3) Real Channel")
    axs[1, 0].axis("off")

    axs[1, 1].imshow(imag_part, cmap="gray")
    axs[1, 1].set_title("3) Imag Channel")
    axs[1, 1].axis("off")
    ...
    # -----------------------------------------------------------------
    # 4) Load the pretrained MRI autoencoder & Encode
    # -----------------------------------------------------------------
    ...
    axs[2, 0].imshow(latents_for_display, cmap="gray")
    axs[2, 0].set_title("4) Latent (One Channel)")
    axs[2, 0].axis("off")
    ...
    # Display decoded magnitude
    axs[2, 1].imshow(np.abs(recon_complex_image[0]), cmap="gray")
    axs[2, 1].set_title("5) Decoded (Magnitude)")
    axs[2, 1].axis("off")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Encode and decode a complex MRI image using a pretrained AutoencoderKL, "
                    "visualizing each step for clarity."
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
