...
def load_model(model_type: ModelType, **overrides: Any) -> nn.Module:
    """
    Load a specific model type (VAE or DiT) using Hydra-based configuration management.

    This function initializes Hydra with a config directory derived from the 'PROJECT_ROOT'
    environment variable. It then composes the config for either a 'vae' or 'dit' model,
    loads it, and returns the instantiated model.

    Args:
        model_type (ModelType): An enum value indicating which model to load (e.g., ModelType.VAE or ModelType.DIT).
    ...
    # Dynamically compose the configuration based on the model_type
    ...
    elif model_type == ModelType.DIT:
        with initialize_config_dir(config_dir=str(Path(os.environ["PROJECT_ROOT"], "configs", "models"))):
            cfg = compose(config_name="dit")
        return load_dit(cfg, **overrides)
...
def apply_overrides(cfg: DictConfig, overrides: Dict[str, Any]) -> DictConfig:
    ...
    for key, value in overrides.items():
        for parent_key, section in cfg.items():
            ...
            # If we never break, the key doesn't exist
            raise KeyError(f"Override key '{key}' not found in any top-level config section.")
...
if __name__ == "__main__":
    """
    Example usage of the load_model function to instantiate a VAE and a DiT model.
    """
