...
class DiTBlockMultiModal(nn.Module):
    ...
    def __init__(
        ...
        self.adaLN_mod_img = nn.Sequential(
            nn.GELU(),
            nn.Linear(dim_img, 7 * dim_img, bias=True)
        )
        ...
        #     nn.GELU(),
        #     nn.Linear(dim_tab, 7 * dim_tab, bias=True)
        # )
    ...
    def forward(
        ...
        # # ============ Image path =============
        # shift_msa_i, scale_msa_i, gate_msa_i, shift_mlp_i, scale_mlp_i, gate_mlp_i, gate_ca_i = \
        #     self.adaLN_mod_img(t_img).chunk(7, dim=1)
        #
        # # Self-attn
        # x_img_sa = self.attn_img(
        #     modulate(self.norm1_img(x_img), shift_msa_i, scale_msa_i)
        # )
        # x_img = x_img + gate_msa_i.unsqueeze(1) * x_img_sa
        #
        # # ============ Table path =============
        # shift_msa_t, scale_msa_t, gate_msa_t, shift_mlp_t, scale_mlp_t, gate_mlp_t, gate_ca_t = \
        #     self.adaLN_mod_tab(t_tab).chunk(7, dim=1)
        #
        # # Self-attn
        # x_tab_sa = self.attn_tab(
        #     modulate(self.norm1_tab(x_tab), shift_msa_t, scale_msa_t)
        # )
        # x_tab = x_tab + gate_msa_t.unsqueeze(1) * x_tab_sa
        #
        # # == 3) Parallel cross-attention ==
        # x_img_norm = self.norm2_img(x_img)
        # x_tab_norm = self.norm2_tab(x_tab)
        #
        # # Cross-attn outputs
        # x_img_ca = self.cross_attn_img(x_img_norm, x_tab_norm)
        # x_tab_ca = self.cross_attn_tab(x_tab_norm, x_img_norm)
        #
        # # Gated cross-attn updates
        # x_img = x_img + gate_ca_i.unsqueeze(1) * x_img_ca
        # x_tab = x_tab + gate_ca_t.unsqueeze(1) * x_tab_ca
        #
        # # == 4) MLP for each modality ==
        # x_img_ff = self.mlp_img(
        #     modulate(self.norm3_img(x_img), shift_mlp_i, scale_mlp_i)
        # )
        # x_img = x_img + gate_mlp_i.unsqueeze(1) * x_img_ff
        #
        # x_tab_ff = self.mlp_tab(
        #     modulate(self.norm3_tab(x_tab), shift_mlp_t, scale_mlp_t)
        # )
        # x_tab = x_tab + gate_mlp_t.unsqueeze(1) * x_tab_ff
        ...
        # Optionally skip cross-attn_img if you only want purely image self-attn:
        # x_img = x_img + self.cross_attn_img(self.norm2_img(x_img), ???)
        #
        # But since we don't have x_tab, you might just remove cross_attn_img usage:
        # For a minimal test, either skip it or do a dummy pass with x_img as cond.

        x_img = x_img + self.cross_attn_img(
            self.norm2_img(x_img),
            x_img  # or pass x_img as "cond" if you want a dummy cross
        )
        ...
        return x_img, None  # Return None for the table path
...
class MultiModalDiT(nn.Module):
    ...
    def __init__(
        ...
        # -- 1) Image patch embed
        # self.img_patch_embed = PatchEmbed2D(
        #     img_height=img_height,
        #     img_width=img_width,
        #     patch_size=img_patch_size,
        #     in_chans=img_in_channels,
    ...
    def initialize_weights(self):
        """
        Initialize modules similarly to original DiT.
        """
        # time embedder
        ...
        gs_img = int(self.img_num_patches**0.5)  # assume H/patch_size == W/patch_size
        pos2d = get_2d_sincos_pos_embed(self.hidden_dim, gs_img)  # shape (Nimg, D)
        assert pos2d.shape[0] == self.img_num_patches
        self.pos_embed_img[0, :, :] = torch.from_numpy(pos2d)

        # fill pos_embed_tab with 1D sin/cos
        # pos1d_tab = get_1d_sincos_pos_embed(self.hidden_dim, self.tab_num_patches)  # (Ntab, D)
        ...
        # pooled mlp
        for p in self.pooled_img_emb_process.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)
    ...
    def forward_without_cfg(
        ...
        # x_tab: torch.Tensor,  # (B, table_width) or (B, 1, 1, table_width), depending on your patch embed
        ...
        if img_tokens.shape[1] == self.img_num_patches:
            ...
            # handle mismatch, e.g. interpolation if needed
            pass
        ...
        # 6) Patch Mixer (image only)
        if self.use_patch_mixer_img and self.patch_mixer_img is not None:
            img_tokens = self.patch_mixer_map_xin_img(img_tokens)
            # e.g. pass pooled_img as cond
            cond_img = self.patch_mixer_map_y_img(pooled_img.unsqueeze(1))
            for blk in self.patch_mixer_img:
                img_tokens = blk(img_tokens, cond_img.squeeze(1), t_emb_img)

            # Mask AFTER patch mixer
            mask_img, ids_restore_img = None, None
            if mask_ratio_img > 0.0:
                n_img_patches = img_tokens.shape[1]
                mask_dict_img = get_mask(B, n_img_patches, mask_ratio_img, device)
        ...
        x_img_final = img_tokens
        for blk in self.blocks:
            x_img_final, _ = blk(x_img_final, None, t_emb_img, None)
...
if __name__ == "__main__":
    ...
    for i, blk in enumerate(net.blocks):
        ...
        print(f"Block {i}: Image MLP -> {mlp_img_type}")
