...
class MultiModalDiTBlock(nn.Module):
    ...
    def __init__(
        ...
        # Gating input for the image side is [t_emb, tab_pooled].
        # We have 3 sub-layers => each sub-layer needs SHIFT + SCALE + GATE => 3×(3×dim) = 9×dim.
        self.adaLN_modulation_x = nn.Sequential(
            nn.GELU(),
            nn.Linear(time_emb_dim + tab_pooled_dim, 9 * dim)
        )
        ...
        # Gating input for the tab side is [t_emb, img_pooled].
        self.adaLN_modulation_t = nn.Sequential(
            nn.GELU(),
            nn.Linear(time_emb_dim + img_pooled_dim, 9 * dim)
        )
    ...
    def forward(
        ...
        x_img_pooled: torch.Tensor,  # (B, dim)
        x_tab_pooled: torch.Tensor,  # (B, dim)
        t_emb: torch.Tensor        # (B, time_emb_dim)
    ):
        """
        We do:
          1. Self-Attn on x_img, then cross-attn from x_img -> x_tab,
             then MLP. Each sub-layer gets shift/scale/gate from adaLN_modulation_x.
          2. Self-Attn on x_tab, then cross-attn from x_tab -> x_img,
             then MLP. Each sub-layer gets shift/scale/gate from adaLN_modulation_t.
        We use updated tokens for cross-attn (no more stale tokens).
        """
        # --------------------------------------------------------------------
        # Gating for the IMAGE side => 9×dim
        #    sub-layer #1 => (shift_msa_x, scale_msa_x, gate_msa_x)
        #    sub-layer #2 => (shift_cross_x, scale_cross_x, gate_cross_x)
        #    sub-layer #3 => (shift_mlp_x, scale_mlp_x, gate_mlp_x)
        # --------------------------------------------------------------------
        combined_img = torch.cat([t_emb, x_tab_pooled], dim=1)  # (B, time_emb_dim + tab_pooled_dim)
        shiftscalesgates_x = self.adaLN_modulation_x(combined_img)
        (
            shift_msa_x, scale_msa_x, gate_msa_x,
            shift_cross_x, scale_cross_x, gate_cross_x,
            shift_mlp_x, scale_mlp_x, gate_mlp_x
        ) = shiftscalesgates_x.chunk(9, dim=1)
        ...
        # -- (2) Cross-Attn from image -> tab (use *updated* x_tab!)
        x_ln2 = modulate(self.norm2_x(x_img), shift_cross_x, scale_cross_x)
        x_img = x_img + gate_cross_x.unsqueeze(1) * self.cross_attn_x_to_tab(x_ln2, x_tab)
        ...
        # --------------------------------------------------------------------
        # Gating for the TAB side => 9×dim
        #    sub-layer #1 => (shift_msa_t, scale_msa_t, gate_msa_t)
        #    sub-layer #2 => (shift_cross_t, scale_cross_t, gate_cross_t)
        #    sub-layer #3 => (shift_mlp_t, scale_mlp_t, gate_mlp_t)
        # --------------------------------------------------------------------
        combined_tab = torch.cat([t_emb, x_img_pooled], dim=1)  # (B, time_emb_dim + img_pooled_dim)
        shiftscalesgates_t = self.adaLN_modulation_t(combined_tab)
        (
            shift_msa_t, scale_msa_t, gate_msa_t,
            shift_cross_t, scale_cross_t, gate_cross_t,
            shift_mlp_t, scale_mlp_t, gate_mlp_t
        ) = shiftscalesgates_t.chunk(9, dim=1)

        # -- (1) Self-Attn on tab
        t_ln = modulate(self.norm1_t(x_tab), shift_msa_t, scale_msa_t)
        x_tab = x_tab + gate_msa_t.unsqueeze(1) * self.attn_t(t_ln)

        # -- (2) Cross-Attn from tab -> image (use *updated* x_img!)
        t_ln2 = modulate(self.norm2_t(x_tab), shift_cross_t, scale_cross_t)
        x_tab = x_tab + gate_cross_t.unsqueeze(1) * self.cross_attn_t_to_img(t_ln2, x_img)
...
class MultiModalDiT(nn.Module):
    ...
    def forward_without_cfg(
        ...
        if self.patch_mixer is not None:
            img_tokens = self.patch_mixer_map_xin(img_tokens)
            for blk in self.patch_mixer:
                img_tokens = blk(img_tokens, t_emb)
            img_tokens = self.patch_mixer_map_xout(img_tokens)

        # 3) Tab => grouping => preproc => pooled
        tab_tokens = self.table_proj(x_tab)
        tab_tokens = self.table_preproc(tab_tokens)
        # TODO: # (DON'T do any pooling yet - we'll do that per-block)

        img_pooled = self.image_pooler(img_tokens)
        ...
        for blk in self.blocks:
            # # Re-pool the image tokens -> (B, dim)
            # img_pooled = self.image_pooler(img_tokens)
            # # Re-pool the tab tokens -> (B, dim) or however you choose
            # tab_pooled = tab_tokens.mean(dim=1)
