...
class MultiModalDiffusion(nn.Module):
    ...
    def sample(
        ...
        c = self.dit.in_channels  # e.g. 3 or 4 for image channels
        T_tab = 10

        x_img = torch.randn(batch_size, c, height, width, device=device, dtype=torch.float64)
        if table_data is None:
            # unconditional tab => random
            x_tab = torch.randn(batch_size, T_tab, device=device, dtype=torch.float64)
            table_is_uncond = True
        else:
            # "conditional" or partial: if you want tab to remain fixed => set sigma=0
            # or if you want to noise it => do x_tab = table_data + t_vals[0]*randn
            x_tab = table_data.to(device, dtype=torch.float64)
            table_is_uncond = False

        # create the time schedule
        t_vals = self.create_edm_timesteps(steps, device)  # shape => (steps+1,)
        # multiply latents by the largest sigma for the start
        x_img *= t_vals[0]
        if table_is_uncond:
            x_tab *= t_vals[0]
        # else if you want partial noise in the conditional tab, do something like:
        # x_tab = x_tab + t_vals[0]*torch.randn_like(x_tab)

        # We'll store these in double precision for numerical stability
        x_img_next = x_img
        x_tab_next = x_tab

        # ---------------------------------------------------------
        # 2) Sampler loop with Heun or Euler steps
        # ---------------------------------------------------------
        for i, (t_cur, t_next) in enumerate(zip(t_vals[:-1], t_vals[1:])):
            ...
            if table_is_uncond:
                ...
                x_t_hat = x_tab_next  # keep the tab the same if we're strictly conditioning
                # or if partial, add noise here too
            ...
            x_img_denoised, x_tab_denoised = self.model_forward_edm(
                x_i_hat, x_t_hat, t_hat, cfg, table_is_uncond
            )
            ...
            # -----------------------------------------------------
            # 2c) 2nd pass (Heun correction)
            # -----------------------------------------------------
            if i < steps - 1:
                x_img_denoised2, x_tab_denoised2 = self.model_forward_edm(
                    x_img_next, x_tab_next, t_next, cfg, table_is_uncond
                )
    ...
    def model_forward_edm(
        ...
        Here we unify image & tab latents, scale them by c_in, and combine
        the predictions with c_skip, c_out.
        """
        B = x_img_in.shape[0]
        sigma_img = torch.full((B, 1, 1, 1), sigma_val, device=x_img_in.device, dtype=torch.float32)

        # EDM factors for image
        ...
        # If table is unconditional => we noise it the same as images.
        # If partial condition => set sigma_tab=0, or do something else.
        if table_is_uncond:
            sigma_tab = sigma_img.view(B, 1)  # shape (B,1)
            c_in_tab = 1.0 / (sigma_tab ** 2 + self.sigma_data ** 2).sqrt()
            c_skip_t = self.sigma_data ** 2 / (sigma_tab ** 2 + self.sigma_data ** 2)
            c_out_t = sigma_tab * self.sigma_data / (sigma_tab ** 2 + self.sigma_data ** 2).sqrt()
        else:
            # purely "condition" => no noise scale
            sigma_tab = torch.zeros_like(sigma_img.view(B, 1))
            c_in_tab = 1.0  # or 1.0 / s_data if you want
            c_skip_t = 1.0
            c_out_t = 0.0

        # Scale the inputs for the forward pass
        x_img_scaled = c_in * x_img_in.float()
        x_tab_scaled = c_in_tab * x_tab_in.float()

        # Forward pass in the new DiT
        out = self.dit.forward(
            x_noisy_img=x_img_scaled,
            x_noisy_tab=x_tab_scaled,
            t=t_embed,
            mask_ratio=0.0  # or some ratio
        )
        ...
        if Fx_tab is not None:
            denoised_tab = c_skip_t * x_tab_in + c_out_t * Fx_tab
