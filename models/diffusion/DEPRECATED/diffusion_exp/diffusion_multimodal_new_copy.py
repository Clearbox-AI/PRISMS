...
class MultiModalDiffusion(ComposerModel):
    ...
    def edm_loss(
        ...
        **kwargs
        ...
        x_noisy_img = x_img + torch.randn_like(x_img) * sigma
        ...
        loss_img = weight * (denoised_img - x_img) ** 2
        loss_img_mean = loss_img.mean(dim=(1, 2, 3))  # average per-sample
        loss = {"loss": loss_img_mean.mean(), "loss_img": loss_img_mean.mean()}
        return loss
    ...
    def generate(
        ...
        final_img = self.edm_sampler_loop(
            x_img,
            # x_tab,
            scenario=scenario,
